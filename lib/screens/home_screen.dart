...
//   // ⭐ 新增：存储从数据库拉取回来的“真实的、已打卡的调查点”
//   List<TaskPoint> _actualSurveyPoints = [];
//
//   // ⭐ 新增：记录当前任务信息，方便刷新时重新请求
//   Map<String, dynamic>? _currentTaskInfo;
...
//
//   // ⭐ 新增方法：tip 静默或带 Loading 刷新当前任务的 GeoJSON (包括规划线、点、实际点)
//   Future<void> _refreshTaskMapData({bool showLoading = false}) async {
//     if (_currentTaskInfo == null) return;
//
//     if (showLoading) {
//       showDialog(
//         context: context,
...
//         Uri.parse('$baseUrl/user/tasks/${_currentTaskInfo!['id']}/detail'),
//         headers: {'Authorization': 'Bearer $token'},
//       );
//
//       if (showLoading && mounted) Navigator.pop(context); // 关掉 Loading
//
...
//
//         // 重新调用加载方法，解析并重绘
//         _loadTaskToMap({'task_info': _currentTaskInfo, 'detail_data': geoJsonData});
//       }
//     } catch (e) {
//       if (showLoading && mounted) Navigator.pop(context);
//       debugPrint("刷新地图数据失败: $e");
//     }
//   }
//
//   // ======== tip 新增点击实际点请求详情的方法========
//   Future<void> _fetchAndEditPoint(TaskPoint point) async {
//     // 假设 GeoJSON 属性里绑定了后端的数据库主键 point_id
//     String pointId = point.properties['point_id'].toString();
//
//     showDialog(
//       context: context,
//       barrierDismissible: false,
...
//       final String baseUrl = ApiConfig.baseUrl;
//
//       // ⭐ 1. 请求后端获取该点位的详细表单 JSON 与照片 URL 列表
//       var response = await http.get(
//         Uri.parse('$baseUrl/user/points/$pointId/properties'),
//         // 请替换为你实际获取单点详情的 API
//         headers: {'Authorization': 'Bearer $token'},
//       );
//
...
//
//       if (response.statusCode == 200) {
//         // 解析后端的明细数据 (包含 properties 大JSON 和 照片链接列表)
//         Map<String, dynamic> pointDetail = jsonDecode(response.body)['data'];
//
//         // ⭐ 2. 将详情传递给填表页，开启【编辑/查看模式】
...
//               editPointId: pointId,
//               initialData: pointDetail, // 把后端存的大JSON原封不动传过去
//             ),
//           ),
//         );
//
//         // ⭐ 3. 如果在表单页点击了“完成提交(更新)”，返回后刷新地图上的打点
//         if (result != null) {
//           _refreshTaskMapData(showLoading: true);
//         }
//       } else {
//         ScaffoldMessenger.of(
//           context,
//         ).showSnackBar(SnackBar(content: Text('获取点详情失败')));
//       }
//     } catch (e) {
...
//       }
//     }
//     // 3. 解析实际打点图层 actual_point_geoms (新增)
//     final actualPointGeoms = detailData['actual_point_geoms'];
//     // if (actualPointGeoms != null &&
//     //     actualPointGeoms['type'] == 'FeatureCollection' &&
//     //     actualPointGeoms['features'] != null) {
//     //   for (var feature in actualPointGeoms['features']) {
//     //     final geometry = feature['geometry'];
//     //     final properties = feature['properties'] ?? {};
//     //
//     //     // 实际打点数据理论上都是 Point 类型
//     //     if (geometry['type'] == 'Point') {
//     //       var coord = geometry['coordinates'];
//     //       // GeoJSON 标准是 [lon, lat]，LatLng 需要 [lat, lon]
//     //       LatLng ll = LatLng(coord[1], coord[0]);
//     //
//     //       // 将解析出的点及其属性（id, path_id, type, point_serial 等）存入列表
//     //       parsedActualPoints.add(
//     //         TaskPoint(location: ll, properties: properties),
//     //       );
//     //
//     //       // 将这些点也纳入地图缩放视野计算范围
//     //       allCoordinatesForBounding.add(ll);
...
//     // }
//     // 注意：这里直接判断它是否为 List
//     if (actualPointGeoms != null && actualPointGeoms is List) {
//       for (var feature in actualPointGeoms) {
//         // 这里的 feature 对应 List 里的每一个 {} 对象
//         final geometry = feature['geometry'];
//         final properties = feature['properties'] ?? {};
//
...
//             );
//
//             parsedActualPoints.add(TaskPoint(
//               location: ll,
...
//       _actualSurveyPoints = parsedActualPoints; // ⭐ 更新真实点状态
//       _currentTaskTitle =
...
//       );
//       // 调用咱们上文写的静默刷新地图数据方法
//       _refreshTaskMapData(showLoading: false);
//     }
//   }
//
//   @override
...
//                     // ⭐ 渲染实际的、从数据库返回的调查点 (_actualSurveyPoints)
//                     ..._actualSurveyPoints.map(
//                       (taskPoint) => Marker(
//                         point: _wgsToGcj(taskPoint.location),
//                         // 注意你之前的 WGS 转 GCJ
//                         width: 45,
//                         height: 45,
//                         child: GestureDetector(
//                           // ⭐ 点击时调用查看/编辑接口
//                           onTap: () => _fetchAndEditPoint(taskPoint),
//                           child: Icon(
//                             Icons.where_to_vote,
//                             color: Colors.orange,
//                             size: 40,
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Color _getTemplateColor(String? type) {
    switch (type) {
      ...
      case '5':
        return Colors.teal;
      default:
        return Colors.orange; // 默认或者没传类型的兜底颜色
    }
  }
  ...
  Future<void> _refreshTaskMapData({bool showLoading = false}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // 重新调用加载方法，解析并重绘
        _loadTaskToMap({
          'task_info': _currentTaskInfo,
          'detail_data': geoJsonData,
        });
      }
    } catch (e) {
      if (showLoading && mounted) Navigator.pop(context);
  ...
  Future<void> _initCurrentLocation() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // ⭐ 修复点 3：初始化镜头居中时，一定要把 WGS84 转化成 GCJ02 再移动相机！
        _mapController.move(_wgsToGcj(_currentLocation!), 16.0);
      }
    } catch (e) {
      debugPrint("初始定位获取失败: $e");
  ...
  void _loadTaskToMap(Map<String, dynamic> result) {
    ...
    // ⭐ 修复点 2: 加载最新数据前，抓取旧的已经选中的线索的唯一标识
    String? oldSelectedLineId =
        _selectedLine?.properties['线路号']?.toString() ??
        _selectedLine?.properties['Id']?.toString();
    ...
    if (actualPointGeoms != null && actualPointGeoms is List) {
      for (var feature in actualPointGeoms) {
        ...
        if (geometry != null && geometry['type'] == 'Point') {
          ...
          if (coord != null && coord is List && coord.length >= 2) {
            // [0] 是经度 lon, [1] 是纬度 lat
            LatLng ll = LatLng(coord[1].toDouble(), coord[0].toDouble());
    ...
    setState(() {
      ...
      // ⭐ 修复点 2：我们在这里不直接置空，而是通过 ID 与新数据对比，努力保住之前的选中状态
      if (oldSelectedLineId != null) {
        try {
          _selectedLine = _taskLines.firstWhere((line) {
            String? currentId =
                line.properties['线路号']?.toString() ??
                line.properties['Id']?.toString();
            return currentId == oldSelectedLineId;
          });
        } catch (e) {
          _selectedLine = null; // 没找到（例如切换了任务），那就老老实实置空
        }
      } else {
        _selectedLine = null;
      }
              ...
              children: [
                ...
                MarkerLayer(
                  markers: [
                    ...
                    // 2. 真实打卡点
                    // ⭐ 渲染实际的、从数据库返回的调查点 (_actualSurveyPoints)
                    ..._actualSurveyPoints.map((taskPoint) {
                      // ⭐ 修复点1：提取类型并赋予它对应的视觉颜色（兼容可能叫 'type' 或 'point_type'）
                      String? pointType =
                          taskPoint.properties['type']?.toString() ??
                          taskPoint.properties['point_type']?.toString();
                      ...
                      return Marker(
                        ...
                        child: GestureDetector(
                          ...
                          child: Icon(
                            Icons.where_to_vote,
                            color: _getTemplateColor(pointType),
                            // ⭐ 在这里使用彩色映射！
                            size: 40,
                          ),
                        ),
                ...
                _isSurveying
                    ...
                    : FloatingActionButton.extended(
                        ...
                        onPressed: () {
                          if (_selectedLine == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text("请先点击地图选择一条蓝色的规划路线")),
                            );
                            return; // 拒绝开始
                          }
                          _startSurvey(); // 开始调查
